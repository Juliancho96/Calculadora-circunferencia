...
function circle(scaleFactor = scale) {
        ...
        ctx.lineWidth = 1 / scaleFactor; // Ajustar el grosor según el zoom
        const gridSize = 50;
    
        const startX = Math.floor(-arrastrarX / gridSize) * gridSize;
        const startY = Math.floor(-arrastrarY / gridSize) * gridSize;
    
        for (let x = startX; x < scaledWidth - arrastrarX; x += gridSize) {
            ctx.beginPath();
            ctx.moveTo(x + arrastrarX, 0);
            ctx.lineTo(x + arrastrarX, scaledHeight);
            ctx.stroke();
        }
    
        for (let y = startY; y < scaledHeight - arrastrarY; y += gridSize) {
            ctx.beginPath();
            ctx.moveTo(0, y + arrastrarY);
            ctx.lineTo(scaledWidth, y + arrastrarY);
            ctx.stroke();
        }
        ctx.restore();
